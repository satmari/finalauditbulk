...
class ControllerDefect extends Controller {
	...
	public function by_garment($garment_name)
	{
		//
		try {
			$defects = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM defect WHERE (garment_name = '".$garment_name."' AND deleted = 0) ORDER BY id asc"));
			...
			return view('defect.index', compact('defects','garment','batch','num'));
		}
		catch (\Illuminate\Database\QueryException $e) {
	...
	public function newdefect($garment_name) {
		...
		try {
			...
			return Redirect::to('/defect_new/'.$garment_name);
		}
	...
	public function delete($id) {
		...
		try {
			...
			return Redirect::to('/defect/delete/'.$id);
		}
	}
